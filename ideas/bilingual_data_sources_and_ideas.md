...
| **المعجم الطبي الموحد (UMD) - WHO** | [umd.emro.who.int](https://umd.emro.who.int/WHODictionary/) | قاموس طبي موحد (إنجليزي-عربي-فرنسي) | **تنزيل Excel/PDF/قاعدة بيانات** | المصدر الذهبي — آلاف المصطلحات معتمدة من WHO |
| **MedDRA** | [meddra.org](https://www.meddra.org/) | معجم طبي دولي للأنشطة التنظيمية | تنزيل PDF | مستخدم في التسجيل الدوائي والسلامة الدوائية globally |
| **WebTeb (ويب طب)** | [webteb.com](https://www.webteb.com/) | محتوى صحي عربي شامل | كشط البيانات | مقالات طبية مترجمة |
| **Archive.org - UMD الكامل** | [archive.org/details/umdwho](https://archive.org/details/umdwho) | المعجم الطبي الموحد كاملاً | تنزيل (1.6 GB) | نسخة أرشيفية كاملة |
| **Desert Sky Arabic Medicine** | [arabic.desert-sky.net](https://arabic.desert-sky.net/) | مفردات الطب العربية | كشط البيانات | مصطلحات طبية مع تصنيفات |
| **Birzeit University Medical Dictionary** | [catalogkoha.birzeit.edu](https://catalogkoha.birzeit.edu/cgi-bin/koha/opac-detail.pl?biblionumber=44516) | قاموس المصطلحات الطبية (إنجليزي-عربي) | تنزيل أو طلب من المكتبة | مصدر أكاديمي موثوق |
| **Reverso Context Medical** | [context.reverso.net](https://context.reverso.net/translation/english-arabic/) | أمثلة مصطلحات طبية في سياق جمل | كشط البيانات | أمثلة استخدام حقيقية في جمل |
...
## 🎯 الخطوات التالية وخارطة التطوير (Roadmap)

### المرحلة 1: مصادر جاهزة للتحميل الفوري

| الأولوية | المصدر | الحجم المتوقع | الصيغة | خطوات التنفيذ |
|:---:|---------|:---:|:---:|---|
| 1 | **WHO UMD** (المعجم الطبي الموحد) | ~150K مصطلح | Excel/PDF | 1. تحميل من `umd.emro.who.int` 2. استخراج أزواج EN-AR 3. تنظيف وتطبيع 4. استيراد عبر `src/auto_ingestion.py` |
| 2 | **MedDRA** | ~100K مصطلح | PDF/Excel | 1. تحميل من الموقع الرسمي 2. استخراج المصطلحات العربية-الإنجليزية 3. ربط بالتخصصات الطبية |
| 3 | **Hitti's Dictionary (Text)** | ~8,400 مصطلح | نص عادي | 1. تحميل ملف `.txt` من Archive.org 2. تحليل الهيكل (إنجليزي-عربي + عربي-إنجليزي) 3. استخراج الأزواج |
| 4 | **MedlinePlus Arabic PDFs** | ~2,000+ مصطلح | PDF | 1. تحميل كتيبات التطعيمات والأمراض 2. استخراج النص ثنائي اللغة بـ `pdfplumber` 3. بناء أزواج موازية |

### المرحلة 2: كشط تلقائي (Web Scraping)

| الأولوية | الموقع | الأداة | المخرجات المتوقعة | الصعوبة |
|:---:|--------|--------|:---:|:---:|
| 1 | **Altibbi** (مصطلحات طبية) | Scrapy + BeautifulSoup | ~10,000+ مصطلح | متوسطة — جداول منظمة |
| 2 | **Almaany** (قاموس طبي) | Scrapy | ~15,000+ مصطلح | متوسطة — فهرس أبجدي |
| 3 | **tbeeb.net** (قاموس طبي) | BeautifulSoup | ~5,000+ مصطلح | سهلة — واجهة بسيطة |
| 4 | **WebTeb** (مقالات طبية) | Puppeteer | جمل موازية | صعبة — محتوى ديناميكي |
| 5 | **translateonline.org** | BeautifulSoup | ~3,000+ مصطلح | سهلة |

> كل سكريبت كشط يُنشأ كـ **إضافة (Plugin)** في نظام `src/plugin_manager.py` — مما يسمح بتوسيع المصادر بدون تعديل الكود الأساسي.

### المرحلة 3: أنابيب معالجة متقدمة (Advanced Pipelines)

#### 3.1 سكريبت معالجة دفعية للنشرات الدوائية (Batch PDF Pipeline)
```
المسار: scripts/batch_leaflet_processor.py
المدخلات: مجلد يحتوي على نشرات دوائية ثنائية اللغة (PDF)
المعالجة:
  1. PyMuPDF لاستخراج النص الإنجليزي
  2. Tesseract OCR (ara+eng) لاستخراج النص العربي من الصور الممسوحة ضوئياً
  3. محاذاة الجمل (Sentence Alignment) بين EN و AR
  4. استخراج المصطلحات الفردية من كل قسم (Composition, Mechanism, Side Effects, ...)
  5. تنظيف عبر src/data_cleaner.py
  6. استيراد عبر src/auto_ingestion.py
المخرجات:
  - glossaries/{product_name}.csv (مصطلحات)
  - corpus_sources/{product_name}_sentences.csv (جمل موازية)
  - إحصائيات الجودة لكل منتج
```

#### 3.2 سكريبت تحميل WHO UMD
```
المسار: scripts/download_umd.py
الوظيفة:
  1. تحميل ملف Excel/PDF من umd.emro.who.int
  2. استخراج الأعمدة الإنجليزية والعربية
  3. إزالة التكرارات وتطبيع المصطلحات
  4. تصنيف حسب التخصص الطبي
  5. حفظ بتنسيق متوافق مع all_pairs.csv
  6. اختيارياً: رفع إلى HuggingFace Dataset
```

#### 3.3 نظام محاذاة الجمل (Sentence Aligner)
```
المسار: src/sentence_aligner.py
الوظيفة:
  - محاذاة تلقائية بين النصوص الإنجليزية والعربية من النشرات الدوائية
  - خوارزميات: طول الجملة، رقم القسم، ترتيب الظهور
  - مخرج: أزواج جمل موازية للتدريب على نماذج الترجمة
```

### المرحلة 4: تكامل مع نماذج الذكاء الاصطناعي

| الفكرة | الوصف | الأدوات |
|---------|-------|--------|
| **تحسين الترجمات بالـ LLM** | مراجعة وتصحيح ترجمات المصطلحات تلقائياً باستخدام GPT/Qwen | OpenAI API أو محلي |
| **كشف الترجمات الخاطئة** | نموذج تصنيف لتحديد المصطلحات التي قد تحتوي على أخطاء | HuggingFace Transformers |
| **توليد مصطلحات جديدة** | استخدام LLM لتوليد ترجمات لمصطلحات غير مغطاة | Prompt Engineering |
| **تضمينات (Embeddings)** | تحويل المصطلحات إلى متجهات للبحث الدلالي | sentence-transformers, AraBERT |
| **نموذج ترجمة طبية مخصص** | fine-tune نموذج Seq2Seq على المسرد | TrOCR / Qwen2-VL (موجود في training/fine_tune.py) |

### المرحلة 5: نشر وتوسيع

| الفكرة | الوصف |
|---------|-------|
| **HuggingFace Space** | نشر واجهة بحث تفاعلية للمسرد |
| **API عام** | نشر glossary-api على Railway/Fly.io |
| **浏览器 extension** | إضافة متصفح لعرض ترجمة المصطلحات الطبية عند التحديد |
| **Telegram Bot** | بوت يترجم المصطلحات الطبية فوراً |
| **دمج مع omni-medical-suite** | ربط المسرد بنظام OCR والتدريب الدقيق |

---

## 📊 تقدير حجم المصادر الجديدة

| المصدر | مصطلحات متوقعة | نوع البيانات | جودة متوقعة |
|---------|:---:|:---:|:---:|
| WHO UMD | ~150,000 | مصطلحات | عالية جداً (معتمد رسمياً) |
| MedDRA | ~100,000 | مصطلحات + تصنيفات | عالية (معيار دولي) |
| Altibbi | ~10,000 | مصطلحات | متوسطة-عالية |
| Almaany | ~15,000 | مصطلحات | متوسطة |
| MedlinePlus Arabic | ~2,000 | جمل + مصطلحات | عالية (NIH) |
| WebTeb | ~5,000 | مقالات + مصطلحات | متوسطة |
| النشرات الدوائية (92 منتج) | ~5,000 | مصطلحات + جمل | عالية (مصدر مباشر) |
| **الإجمالي المتوقع** | **~287,000+** | | |

> مع الإضافة إلى الـ 386,402 موجود حالياً، يمكن أن يصل الإجمالي إلى **~670,000+ زوج** — مما يجعله أكبر مسرد طبي عربي-إنجليزي مفتوح المصدر.

---

## 🔧 أفكار تطوير مرتبطة بنظام Calibre

### استغلال نظام الإضافات (Plugin System)
- إنشاء إضافة `WHOPlugin` لتحميل UMD تلقائياً
- إنشاء `AltibbiPlugin` و `AlmaanyPlugin` للكشط الدوري
- إنشاء `PharmaLeafletPlugin` لمعالجة النشرات الدوائية

### استغلال نظام الاستيراد التلقائي (Auto Ingestion)
- وضع ملفات PDF الجديدة في `corpus_sources/` تتم معالجتها تلقائياً
- مراقبة مجلد `downloads/` لمعالجة أي ملف جديد فوراً

### استغلال نظام التصدير (Multi-Format Export)
- تصدير نسخة HuggingFace لمسرد موسع للتدريب على نماذج الترجمة
- تصدير TMX لاستخدامه في أدوات الترجمة الاحترافية (SDL Trados, memoQ)
- تصدير fastText لتدريب نماذج تمثيل الكلمات الطبية

### استغلال واجهة API (glossary-api)
- إضافة endpoint لاقتراح ترجمات ذكية (LLM-powered)
- إضافة endpoint للبحث الدلالي (Embeddings-based)
- ربط API مع HF Space للعرض التفاعلي